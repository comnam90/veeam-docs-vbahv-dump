...
product: "vbahv"
doc_type: "userguide"
source_url: "https://helpcenter.veeam.com/docs/vbahv/userguide/restore_how_entire.html"
...
The maximum number of disk nodes available on Nutanix AHV VMs for each bus type is limited. Veeam Backup & Replication can attach to a VM up to 6 SATA, 256 SCSI, 4 IDE and 7 PCI disks. If you restore a VM that has more disks of any of those bus types, Nutanix AHV will attach the disks to remaining nodes of other bus types in the default priority: SATA, SCSI, IDE, PCI. You can modify [the Veeam Plug-in for Nutanix AHV configuration](bus_type_restore_order.md), to instruct Nutanix AHV to ignore original bus types and to use a specific order of bus types.
...
To learn how to restore an entire VM, see [Performing VM Restore](restore_to_ahv.md).
